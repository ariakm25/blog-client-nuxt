<template>
  ...
  >
    <template slot="title">
      <a-row>
        <a-col :span="12"
          ><h1>Article Draft ({{ pagination.total }})</h1></a-col
        >
        <a-col :span="12">
    ...
    <template slot="action" slot-scope="text">
      <nuxt-link :to="'/admin/articles/' + text"
      ...
      this.$axios
        ...
        .then((data) => {
          this.$message.success({
            content: `Article ${data.data.data.title} deleted succesfully.`,
            key
          })
          this.fetch()
        })
        .catch(() => {
          this.$message.error({ content: 'Failed to delete article.', key })
        })
    }
  },
  head() {
